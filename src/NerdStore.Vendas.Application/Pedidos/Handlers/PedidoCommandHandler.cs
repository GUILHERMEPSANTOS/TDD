...
namespace NerdStore.Vendas.Application.Pedidos.Handlers
{
    public class PedidoCommandHandler : CommandHandler, IRequestHandler<AdicionarItemCommand, bool>
    {
        ...
        {
            var pedido = await _pedidoRepository.ConsultarPedidoPorClientId(clientId);

            if (pedido is null)
            {
                pedido = Pedido.PedidoFactory.GerarNovoPedido(clientId);
        ...
        {
            var itemExistente = pedido.VerificaSePossuiItem(pedidoItem.ProdutoId);
